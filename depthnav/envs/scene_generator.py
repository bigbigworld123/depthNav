...
class BoxGenerator:
    def __init__(
        ...
        rotation_rng=Uniform([0., 0., 0.], [3.0, 3.0, 3.0]),
        seed=None
    ):
    ...
    def is_inside(self, points):
        coord_inside = th.logical_and(self.low <= points, points <= self.high).to(dtype=th.bool)
    ...
    def sample(self):
        ...
        positions = self.low + (self.high - self.low) * th.rand((num_points, 3), generator=self.gen)
...
class CylinderGenerator:
    ...
    def is_inside(self, points):
        ...
        in_height = th.logical_and(0.0 <= from_center[:,2], from_center[:,2] <= self.height).to(dtype=th.bool)
    ...
    def sample(self):
        ...
        theta = 2. * th.pi * th.rand(num_points, generator=self.gen)
...
class SceneGenerator:
    def __init__(self,
        ...
        self.summary_path = str(list(Path(self.dataset_path).glob("*.scene_dataset_config.json"))[0])
    ...
    def _create_scene_json(self):
        ...
        for id in range(self.num):
            ...
            for generator in self.keep_in_bounds:
                ...
                scene_json["object_instances"].extend(self._create_objects(
    ...
    def _create_objects(self, object_paths, positions, orientations, sizes, ids):
        ...
        for i in range(len(positions)):
            ...
            object_instance["translation"] = std_to_habitat(positions[i], None)[0].tolist()
            object_instance["rotation"] = std_to_habitat(None, orientations[i])[1].tolist()
            if sizes[i].ndim == 0:
...
if __name__ == "__main__":
    ...
    g = SceneGenerator(
        ...
        stage="stages/box_2",
        keep_in_bounds=[
            CylinderGenerator(base_center=[0., 0., 0.], radius=12.5, height=5., dataset_path=dataset_path, object_set="primitives/medium", density=args.density, seed=42)
        ],
        keep_out_bounds=[
