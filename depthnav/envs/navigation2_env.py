...
class Navigation2Env(BaseEnv):
    def __init__(
        ...
        self.target_rng = self._create_rng(
            "target",
            self.random_kwargs,
            default_rng=Uniform([9., 0., 5.], [0., 2.5, 2.5])
        )
        self.target_speed_rng = self._create_rng(
            "target_speed",
            self.random_kwargs,
            default_rng=Uniform([self.target_kwargs.get("target_speed", 5.)], [0.])
        )
    ...
    def reset_agents(self, indices: Optional[List] = None):
        ...
        min_starting_distance_to_target = self.random_kwargs.get("min_starting_distance_to_target", 5.0)
        indices = th.arange(self.num_envs, device=self.device) if indices is None else indices

        # generate position
        position = self.safe_generate(self.position_rng, indices, safe_spawn_radius).to(self.device)

        # generate target
        self._target[indices] = self.safe_generate(self.target_rng, indices, safe_spawn_radius).to(self.device)
        self._target_speed[indices] = self.target_speed_rng.generate((len(indices), 1)).to(self.device)
        ...
        for _ in range(max_iter):
            too_close[indices] = (position - self._target[indices]).norm(dim=1) < min_starting_distance_to_target
        ...
        up = th.tensor([[0., 0., 1.]]).expand(len(indices), 3)
    ...
    def step(self, action: th.Tensor, is_test=False):
        ...
        # else our action type requires us to calculate yaw
            ...
            yaw = cur_yaw + yaw_rate * (1./self.dynamics.ctrl_dt)
        else:
            raise NotImplementedError
        ...
        x_vector = th.tensor([1., 0., 0.], device=device).view(1, 3, 1).expand(self.num_envs, 3, 1)

        # get x-axis vector in world frame
        if self.inertial_frame == Frame.START:
            x_vector_wf = (self.rot_ws.R @ Rz @ x_vector).squeeze(-1) # (N,3,1) -> (3,N)
        elif self.inertial_frame == Frame.WORLD:
            x_vector_wf = (Rz @ x_vector).squeeze(-1) # (N,3,1) -> (3,N)
        elif self.inertial_frame == Frame.BODY:
            x_vector_wf = (self.rot_wb.R @ Rz @ x_vector).squeeze(-1) # (N,3,1) -> (3,N)
        else:
            raise NotImplementedError
    ...
    def get_success(self):
        within_radius = th.norm(self._target - self.position, dim=1) < self.success_radius
    ...
    def get_observation(self):
        ...
        if self.target_type == TargetType.TARGET_VELOCITY:
            ...
            obs["target"] = th.cat([
                target_velocity,
                1./(self.target_distance.clone().detach().clamp(min=0.5))
    ...
    def get_reward(self, action=None) -> th.Tensor:
        ...
        lambda_vmax = self.reward_kwargs.get("lambda_vmax", 0.)
        ...
        # collision loss
        def positive_speed_towards_collision(velocity: th.Tensor, collision_vec: th.Tensor):
        ...
        speed_towards_collision = positive_speed_towards_collision(self.velocity, self.collision_vector)
        distance_penalty = (falloff_dis - (self.collision_dis - self.robot_radius)).relu()**2
        intersection_barrier = th.log(1 + th.exp(beta_2*(self.collision_dis - self.robot_radius))) 
        ...
        loss_v = F.smooth_l1_loss(velocity_difference, th.zeros_like(velocity_difference), reduction="none")
        ...
        desired_yaw_vector = slerp(self.target_direction, avg_vel, t).clone().detach() # works, but should try without detach
    ...
    def target_velocity(self):
        ...
        des_velocity_clamped = des_velocity.norm(dim=1, keepdim=True).clamp(max=self.target_speed.clone().detach()) * F.normalize(des_velocity)
    ...
    def yaw_vector(self):
        ...
        yaw_vec = th.stack([x_axis[:, 0], x_axis[:, 1], th.zeros_like(x_axis[:, 2])], dim=1)
