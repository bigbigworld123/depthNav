...
class BaseEnv:
    def __init__(
        ...
        self.gravity_rng = self._create_rng(
            "gravity",
            self.random_kwargs,
            default_rng=Uniform([0., 0., -9.81], [0., 0., 0.])
        )
        ...
        if visual:
            self.observation_space = spaces.Dict({
                "state": spaces.Box(low=-np.inf, high=np.inf, shape=(self.state_size,), dtype=np.float32)
            })
    ...
    def step(self, action: th.Tensor, target_dir_wf: th.Tensor=None, is_test=False, device="cpu"):
        """
        ...
        target_dir_wf = th.tensor([1., 0., 0.], device=self.device).repeat(self.num_envs, 1) if target_dir_wf is None else target_dir_wf
        ...
        self._done = self._done | self._success | self.is_collision | (self._step_count >= self.max_episode_steps)
        ...
        for indice in range(self.num_envs):
            for metric in loss_metrics:
                ...
                if metric not in self._info[indice]["loss_metrics"]:
                    self._info[indice]["loss_metrics"][metric] = loss_metrics[metric][indice]
                update_running_mean(
            ...
            if self._done[indice]:
                ...
                self._info[indice]["episode_reward"] = self._rewards[indice].cpu().clone().detach().numpy()
                self._info[indice]["episode_avg_step_reward"] = (self._rewards[indice]/max(self._step_count[indice], 1)).cpu().clone().detach().numpy()
                self._info[indice]["episode_length"] = self._step_count[indice].cpu().clone().detach().numpy()
                self._info[indice]["episode_duration"] = (self._step_count[indice] * self.dynamics.ctrl_dt).cpu().clone().detach().numpy()
    ...
    def reset_agents(
        ...
        if indices is None:
            ...
            self._reward = self._reward.scatter(0, indices, th.zeros(len(indices), device=self.device))
            self._rewards = self._rewards.scatter(0, indices, th.zeros(len(indices), device=self.device))
            self._done = self._done.scatter(0, indices, th.zeros(len(indices), dtype=th.bool, device=self.device))
        ...
        pos = self.safe_generate(self.position_rng, indices, 1.0).to(self.device) if pos is None else pos
        vel = self.velocity_rng.generate(size).to(self.device) if vel is None else vel
        gravity = self.gravity_rng.generate(size).to(self.device) if gravity is None else gravity
        start_rot = Rotation3(num=len(indices), device=self.device) if start_rot is None else start_rot
        ...
        if self.visual:
            self.scene_manager.reset_agents(std_positions=pos, std_orientations=rot.to_quat(), indices=indices)
    ...
    def update_observation(self, indices: Optional[int] = None):
        ...
        for sensor_uuid in self._visual_sensor_list:
            if "depth" in sensor_uuid:
                sensor_spec = [sensor for sensor in self._sensor_kwargs if sensor["uuid"] == sensor_uuid][-1]
                near_clip = sensor_spec.get("near", 0.05)
                far_clip = sensor_spec.get("far", 20.0)
                img = th.stack([each_agent_obs[sensor_uuid] for each_agent_obs in img_obs], dim=0).unsqueeze(1)
                ...
                img = th.transpose(th.stack([each_agent_obs[sensor_uuid] for each_agent_obs in img_obs])[..., :3], (0, 3, 1, 2))
            elif "semantic" in sensor_uuid:
                img = th.stack([each_agent_obs[sensor_uuid] for each_agent_obs in img_obs])
            else:
    ...
    @timerlog.timer.timed
    def safe_generate(self, rng: Union[Uniform, Normal], scene_ids, safe_radius, max_iter=1000):
        """
        ...
        for i, scene_id in enumerate(scene_ids):
            for _ in range(max_iter):
                ...
                is_collision = bool(self.scene_manager.get_point_is_collision(
    ...
    def update_collision(self, indices: Optional[List[int]] = None):
        if not self.visual:
            if indices is None or self._collision_point is None:
                ...
                self._collision_point[th.arange(self.dynamics.N),index%3] = self._flatten_bboxes[0][index]
            else:
                value, index = th.hstack([
            ...
            self._is_out_bounds = (self.position < self._bboxes[0][0]).any(dim=1) |\
                                  (self.position > self._bboxes[0][1]).any(dim=1)

        else:
            if indices is None or self._collision_point is None:
                self._collision_point = self.scene_manager.get_collision_point().to(self.device)
            else:
                self._collision_point[indices] = self.scene_manager.get_collision_point(indices=indices.cpu()).to(self.device)
        ...
        self._is_collision = (self._collision_dis < self.robot_radius) | self._is_out_bounds
