...
class Rotation3:
    ...
    def is_valid_rotation(self, atol=1e-5):
        ...
        ortho_check = th.allclose(th.matmul(self._R, self._R.transpose(1, 2)), I, atol=atol)
        normal_check = th.allclose(th.norm(self._R, dim=2), th.ones(self.num, 3, device=self.device), atol=atol)
        det_check = th.allclose(th.det(self._R), th.ones(self.num, device=self.device), atol=atol)
    ...
    def from_quat(self, q: th.tensor, device=th.device("cpu")) -> SelfRotation3:
        ...
        R = th.stack([
            th.stack([w*w + x*x - y*y - z*z, 2*(x*y - w*z), 2*(x*z + w*y)], dim=1),
            th.stack([2*(x*y + w*z), w*w - x*x + y*y - z*z, 2*(y*z - w*x)], dim=1),
            th.stack([2*(x*z - w*y), 2*(y*z + w*x), w*w - x*x - y*y + z*z], dim=1)
        ], dim=-2)
    ...
    def to_quat(self):
        ...
        w = th.cos(phi/2) * th.cos(theta/2) * th.cos(psi/2) + th.sin(phi/2) * th.sin(theta/2) * th.sin(psi/2) 
        x = th.sin(phi/2) * th.cos(theta/2) * th.cos(psi/2) - th.cos(phi/2) * th.sin(theta/2) * th.sin(psi/2) 
        y = th.cos(phi/2) * th.sin(theta/2) * th.cos(psi/2) + th.sin(phi/2) * th.cos(theta/2) * th.sin(psi/2) 
        z = th.cos(phi/2) * th.cos(theta/2) * th.sin(psi/2) - th.sin(phi/2) * th.sin(theta/2) * th.cos(psi/2) 
