...
class Quaternion:
    ...
    def x_axis(self):
        ...
        return th.stack([1 - 2*(y**2 + z**2), 2*(x*y + z*w), 2*(x*z - y*w)])

    @property
    def y_axis(self):
        # second column of R (3,N)
        w, x, y, z = self.w, self.x, self.y, self.z
        return th.stack([2*(x*y + w*z), 1 - 2*(x**2 + z**2), 2*(y*z - w*x)])

    @property
    def z_axis(self):
        # third column of R (3,N)
        w, x, y, z = self.w, self.x, self.y, self.z
        return th.stack([2*(x*z - w*y), 2*(y*z + w*x), 1 - 2*(x**2 + y**2)]) 
    ...
    def as_matrix(self):
        ...
        w, x, y, z = self.w, self.x, self.y, self.z
        return th.stack([
            th.stack([1 - 2*(y**2 + z**2), 2*(x*y - w*z), 2*(x*z + w*y)]),
            th.stack([2*(x*y + w*z), 1 - 2*(x**2 + z**2), 2*(y*z - w*x)]),
            th.stack([2*(x*z - w*y), 2*(y*z + w*x), 1 - 2*(x**2 + y**2)])
        ])
...
def unit_test():
    ...
    q = Quaternion()
    assert q.shape == (4,1)
    assert th.allclose(q.x_axis[0], th.tensor([1., 0., 0.]))
    assert th.allclose(q.y_axis[0], th.tensor([0., 1., 0.]))
    assert th.allclose(q.z_axis[0], th.tensor([0., 0., 1.]))
    ...
    assert th.allclose(input_euler, output_euler, atol=1e-4), abs(input_euler-output_euler)

    # check that 0,0,0 rotvec is the unit quaternion
    q = Quaternion.from_rotvec(th.tensor([[0.,0.,0.]]).T)
